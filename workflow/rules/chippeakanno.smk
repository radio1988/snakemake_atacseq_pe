rule chippeakanno:
    input:
        peak="results/{folder}/{name}.narrowPeak",
        gtf=config['GTF'],
    output:
        "results/{folder}/{name}.narrowPeak.full_anno.xlsx"
    log:
        "results/{folder}/{name}.narrowPeak.full_anno.log"
    benchmark:
        "results/{folder}/{name}.narrowPeak.full_anno.benchmark"
    params:
