...
if (length(args)==0) { # for debug
    ...
    PeakLocForDistance <- args[6]
    FeatureLocForDistance <- args[7]
    oname <- args[8]
}
...
TxDb  
end_time <- Sys.time()
print(paste('Time for parsing GTF:', end_time - start_time))
...
end_time <- Sys.time()
print(paste('Time for parsing peak file:', end_time - start_time))


# transcript level annotation
gr.anno <- annotatePeakInBatch(
    myPeakList = peaks.gr,
    AnnotationData =  transcript.gr,
    PeakLocForDistance= PeakLocForDistance, # default start
    FeatureLocForDistance = FeatureLocForDistance, # default TSS
...
if (CHIPPEAKANNO_MODE == 'both'){
    ...
    peak.anno.df$dedup_id <- paste(peak.anno.df$peak_id,peak.anno.df$gene_name)
    peak.anno.df <- peak.anno.df %>% distinct(dedup_id, .keep_all = TRUE) # keep the first
    peak.anno.df.protein_coding <- subset(peak.anno.df, 
...
collapse_anno <- function(peak.anno.df, outname){
    ...
    if (CHIPPEAKANNO_MODE == 'both'){
        peak.anno.df.collapsed <- data.frame(
            apply(peak.anno.df.collapsed[,1:12], 2, RM),       # not collapse seqnames, start, end, width, strand, conc ...
            peak.anno.df.collapsed[,13:ncol(peak.anno.df.collapsed)])  # CHIPPEAKANNO_MODE, CALLER specific
    } else if(CHIPPEAKANNO_MODE == 'overlapping'){
        peak.anno.df.collapsed <- data.frame(
            apply(peak.anno.df.collapsed[,1:10], 2, RM), 
...
peak.anno.df.collapsed <- collapse_anno(
    peak.anno.df, 
    paste(prefix, "anno.collapsed.xlsx", sep = ".")
)

peak.anno.df.protein_coding.collapsed <- collapse_anno(
    peak.anno.df.protein_coding, 
...
# Anno QC
pdf(file.path(odir, paste(oname, 'anno_stats.1.pdf', sep='.')))
barplot(table(gr.anno$insideFeature))
dev.off()

# TSS QC
pdf(file.path(odir, paste(oname, 'TSS.transcripts.pdf', sep='.')))
...
# TTS QC
pdf(file.path(odir, paste(oname, 'TTS.transcripts.pdf', sep='.')))
binOverFeature(peaks.gr, annotationData=transcript.gr,
               # select = "nearest",
               # PeakLocForDistance="middle",
...
aCR<-assignChromosomeRegion(peaks.gr, 
                            ...
                            TxDb=TxDb)
pdf(file.path(odir, paste(oname,'anno_stats.2.pdf', sep='.')))
